...
class EnvWorker():
    ...
    def get_step_train_dict(self, step_idx):
        """Returns the training data for a specific step."""
        # For 3SPO, we need to extract only the tokens for the action at step_idx.
        # But wait, the input_ids is concatenated.
        # Actually, if we use the full history as prompt, we just need the labels for this step.
    ...
    def reset(self, task_config):
        ...
        self.history_actions = []  # Added for 3SPO replay in osworld
    ...
    def step(self, prediction):
        ...
        if not self.is_done:
            ...
            self.history_actions.append(prediction) # Record the action for replay
            ...
            return {
                ...
                'is_done': self.is_done,
                'format_reward': format_reward,
                'history_actions': self.history_actions.copy() # Return actions for storage
            }
        else:
            self.history_actions.append(prediction) # Record the action for replay
            self.process_message(self.history_messages[-1:]) # gpt answer only
    ...
    def get_obs(self):
        """Returns the current observation messages for VLLM input."""
        return {
            "env_idx": self.worker_idx,
            "obs_messages": self.history_messages,
            "is_done": self.is_done
    ...
    def reset_to_state(self, task_config, action_history):
        """Reset the environment and replay actions to reach a specific state."""
