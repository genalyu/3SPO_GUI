...
class ReplayBuffer():
    ...
    def update_replay_buffer(self, task_config, batch_item, eval_result, history_actions=None):
        task_id = task_config["task_id"]
        # In 3SPO, we might store states even if they aren't fully successful yet
        if eval_result > 0.05:
            task_replay_buffer = self.pos_dataset[task_id]
        else:
            return 

        # Store the history to allow starting from this state
        if history_actions is not None:
            batch_item.non_tensor_batch['history_actions'] = history_actions
    ...
    def update_replay_buffer_batch(self, task_configs, batch, history_actions_list=None):
        eval_results = batch.batch['eval_results'].tolist()

        if history_actions_list is None:
            history_actions_list = [None] * len(task_configs)

        for task_config, batch_item, eval_result, history_actions in zip(task_configs, batch, eval_results, history_actions_list):
            self.update_replay_buffer(task_config, batch_item, eval_result, history_actions)
